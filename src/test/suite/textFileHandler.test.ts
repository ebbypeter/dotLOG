...
suite('TextFileHandler Tests', () => {
  let handler: TextFileHandler;
  let mockEditor: MockDocumentEditor;

  setup(() => {
    mockEditor = new MockDocumentEditor();
    handler = new TextFileHandler(mockEditor);
  });

  suite('canHandle', () => {
    test('should return true for .txt file with .LOG prefix', () => {
      const document = new MockTextDocument('test.txt', 'plaintext', ['.LOG']);
      assert.strictEqual(handler.canHandle(document), true);
    });

    test('should return false for .txt file without .LOG prefix', () => {
      const document = new MockTextDocument('test.txt', 'plaintext', ['Some content']);
      assert.strictEqual(handler.canHandle(document), false);
    });

    test('should return false for non-.txt file even with .LOG prefix', () => {
      const document = new MockTextDocument('test.log', 'log', ['.LOG']);
      assert.strictEqual(handler.canHandle(document), false);
    });

    test('should return false for empty .txt file', () => {
      const document = new MockTextDocument('test.txt', 'plaintext', []);
      assert.strictEqual(handler.canHandle(document), false);
    });

    test('should return false for .txt file with .LOG not as first line', () => {
      const document = new MockTextDocument('test.txt', 'plaintext', ['Some content', '.LOG']);
      assert.strictEqual(handler.canHandle(document), false);
    });
  });

  suite('getFileType', () => {
    test('should return SupportedFileType.TEXT', () => {
      assert.strictEqual(handler.getFileType(), SupportedFileType.TEXT);
    });
  });

  suite('formatTimestamp', () => {
    test('should format timestamp with newlines for plain text', () => {
  ...
  suite('processDocument', () => {
    test('should successfully process document with valid timestamp', async () => {
      const document = new MockTextDocument('test.txt', 'plaintext', ['.LOG']);
      const timestamp = '2025-08-08 14:30';
